...
class Converter:
    ...
    def add_rule(self, pattern, f):
        if pattern in self.processor:
            errors.append(f"Pattern >{pattern}< is used twice")
        self.processor[pattern] = f
    ...
    def build_processor(self):
        try:
            ...
            processor[Message] = Message_proc
            ...
            rotary_pattern = r"C[12]\s*=DC\(" + float_patern + r"\s*\)"
            processor[rotary_pattern] = lambda x: ("A" + x[1] + " B" + x[1], "", "MOVE" )

            # tool selection
            toolno_pattern = r'TC_TOOL_NO\s*\(\s*\"(\d+)\"\s*\)'
            ...
            for key in self.groups:
                ...
                # print(values, key)
                processor[pattern] = lambda x, key=copy(key),values=copy(values), goodline=copy(goodline): (goodline.format(
                    key,
                    x[1],
            ...
            processor[accel_patern] = lambda x: (f' ;acceleration: {x[0]}\n', "", "OK")
    ...
    def process_re(self, line):
        try:
            ...
            self.errors.append(f"Process error {e}")
    ...
    def deprecated_process(self, line ):
...
# class Converter:
#     """
#     This class is used to end-to-end converting
#     """
...
#     def add_rule(self, section, pattern, f):
#         if section in self.processor and pattern in self.processor[section]:
#             errors.append(f"Pattern >{pattern}< is used twice in section >{section}<")
#         if section not in self.processor:
#             self.processor[section] = {}    
#         self.processor[section][pattern] = f
